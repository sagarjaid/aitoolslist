...
const Tool = ({ name, description, title, price, tag1, tag2, tag3, tag4, url, coverImg, website, upvote, id, item }) => {
    ...
    return (
        <>
            <div className='flex flex-col lg:flex-row m-1 rounded-xl'>
                <img className='rounded-sm w-full lg:w-3/5 border' src={checkURL(coverImg) ? coverImg : `${website}${coverImg}`} />
                <div className='flex flex-col gap-4 py-4 lg:p-4 lg:pt-0'>
                    ...
                    <div className='flex justify-between gap-2'>
                        ...
                        <button onClick={handleOpenTool} className='flex max-w-fit justify-center items-center border text-white bg-black text-xs pr-2 pl-3 rounded-full hover:bg-white hover:text-black'>
                            <span>Open Tool</span>
                    ...
                    <div className='text-sm underline cursor-pointer'><a href="/">Submit your tool!</a></div>
                    <div className='text-xs'>* means they have a free plan but its limited</div>
                </div>
            </div>

            <div className='text-xl'>
                {name} Alternative or Similar AI Tools
            </div>
            {!relatedData[0] && <div className="text-lg flex flex-col items-center gap-3 mt-6">
